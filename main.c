...
bool Win = false;   // For player win, affects EndScreen
bool Init_FadeSh = true; // For fading shadowcasters
Color BackColor = RAYWHITE; // Background gamescreen color

int main(void) {
    InitWindow(WindowWidth, WindowHeight, "The Tale of the Labyrinth"); // Opens game window 
    InitAudioDevice();  // Access to speakers
    ...
    Music GameMusic = musics[rand()%4]; // Random music
    ...
    int l=0; // Character index in game logic.
    int Round = 1;  // Number of rounds that players are played

    bool allAreDied = false;
    double TimeEnding; // When all players dead, save that time and waits for 2.5 seconds 
        ...
        switch(Current) {
            ...
            case GameScreen: {
                ...
                switch(State) {
                    ...
                    // Calculate and the max number of charcters.
                        int MaxPlayer = 0;
                        ...
                        int MaxInterimWall = Calculate_Max_Interim_Wall(m, n);  // = max(min(m, n), 1)
                    ...
                    // Get wall count
                        ...
                        while (!WindowShouldClose()) {
                            ...
                            EndDrawing();
                            if (Submit_Button() || IsKeyPressed(KEY_ENTER)) {
                                int sum=0;
                                ...
                                if (sum>=0 && sum<=((m-1)*(n-1))) {nWalls = sum; break;}
                                    ...
                                    double t0 = GetTime();
                                    Draw_Map_Infs_Error(t0, BackColor, str, n, m);
                                }
                            }
                        }

                    // Place randomly valid walls with BFS algorithm
                        WallPro Wall;
                        bool InputWall;
                        for (int i=0; i<nWalls; i++) {
                            ...
                            do {
                                ...
                                if (Checked == m*n) InputAgain = false;
                                else map[(int)W.y][(int)W.x] = 1;
                            } while (InputAgain);
                        }
                        Reset_Map_Blocks_for_Move_Elements(m, n);   // for MoveShc case
                ...
                // Second case: control explorer moves      
                    case MoveExs: { 
                    ...
                    if (!Are_All_Players_Dead())
                        while (!WindowShouldClose() && l<nExplorers) {
                            if (notChoosed) {
                                ...
                                UpdateMusicStream(GameMusic);
                                BeginDrawing();
                                DrawText("   To save: F1\n   To load: F2\n  Ch theme: F3", 899, 225, 20, (Color){205,50,0,255});
            ...
            case EndScreen: { 
                ...
                bool showResult; 
                while (GetTime() - TimeEnding <= 2.5) showResult = false; // Wait for 2.5 seconds when game ends
    ...
    UnloadMusicStream(music1); UnloadMusicStream(music2); 
    UnloadMusicStream(music3); UnloadMusicStream(music4); 
    UnloadMusicStream(EndGameMusic);
    UnloadSound(DieSound); UnloadSound(VictorySound); 
    UnloadSound(MovingSound); UnloadSound(ShadowSound); 
    UnloadSound(GiveGiftSound); UnloadSound(EarthquakeSound); UnloadSound(ForceSound);
